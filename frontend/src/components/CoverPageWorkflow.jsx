...
import {
  COVER_COLOUR_OPTIONS,
  COVER_THEME_CATALOGUE,
  COVER_THEME_SLOT_COUNT,
} from "../theme";
import {
  loadCoverWorkflowState,
  saveCoverWorkflowState,
...
const normalizeLibraryPayload = (library) => {
  const themes = Array.isArray(library?.themes)
    ? library.themes.map((theme) => {
        const rawId =
          typeof theme?.id === "string" ? theme.id.trim() : theme?.id;
        const normalizedId =
          typeof rawId === "string" && /^v\d+/i.test(rawId)
            ? rawId.toUpperCase()
            : rawId || theme?.themeId;
        const frontCover = resolveCoverUrl({ ...theme, id: normalizedId });
        const resolvedCover =
          frontCover || theme?.coverUrl || theme?.thumbnailUrl;
        const coverUrl = resolvedCover ? normalizeAssetUrl(resolvedCover) : "";
        const thumbnailUrl = theme?.thumbnailUrl
          ? normalizeAssetUrl(theme.thumbnailUrl)
          : coverUrl;
        const previewUrl = theme?.previewUrl
          ? normalizeAssetUrl(theme.previewUrl)
          : "";
        return {
          ...theme,
          id: normalizedId,
  ...
  Object.entries(colourSource).forEach(([version, grades]) => {
    ...
    const normalizedVersion = versionKey
  ...
  return {
    ...
    colours,
    colour_versions: (
      library?.colour_versions || Object.keys(colours || {})
...
const mapLibraryToThemes = (library) => {
  ...
  const colourEntries = (
    baseColours.length ? baseColours : COVER_COLOUR_OPTIONS.map((c) => c.id)
  ).map((version, index) => ({
    id: version,
    label: version,
    imageUrl: "",
...
const CoverPageWorkflow = ({
  ...
  const resolvedGradeNames = useMemo(() => {
    ...
    identifiers.forEach((identifier) => {
      result[identifier] = resolveGradeLabel(
        identifier,
        gradeNameSource?.[identifier],
      );
    });
  ...
  const effectiveReadOnly =
    viewReadOnly || (!isAdmin && workflowStatus === "4");
  ...
  const getThemeLabel = useCallback(
    (themeId, fallbackLabel) => {
      ...
      const match = themes.find(
        (theme) => normalizeThemeId(theme.id) === normalized,
      );
  ...
  const getClientBaseline = useCallback(
    (gradeKey) => {
      const entry =
        (clientGrades && clientGrades[gradeKey]) ||
        (fetchedGrades.client && fetchedGrades.client[gradeKey]) ||
        {};
      const cached = baselineByGrade?.[gradeKey] || {};
      const baselineTheme = normalizeThemeId(
        cached.theme || entry?.theme || entry?.theme_id || entry?.clientTheme,
      );
  ...
  const enabledGrades = useMemo(() => {
    ...
    Object.entries(grades).forEach(([key, value]) => {
      ...
      const isEnabled =
        value && typeof value === "object" ? Boolean(value.enabled) : false;
  ...
  const assignedGradeSummaries = useMemo(() => {
    const filteredGrades = GRADE_ORDER.filter((gradeKey) =>
      isGradeEnabled(gradeKey),
    );
    return filteredGrades
      .map((gradeKey) => {
        ...
        const gradeLabel = resolveGradeLabel(
          gradeKey,
          resolvedGradeNames[gradeKey],
        );
        const imageUrl = baselineColour
          ? libraryColours[baselineColour]?.[gradeCode]
          : null;
        return {
          gradeKey,
          gradeLabel,
          colourId: baselineColour,
  ...
  const hydrateSelectionsFromServer = useCallback(async () => {
    ...
    if (
      summaryCacheKey &&
      Object.keys(fetchedGrades.client || {}).length === 0
    ) {
      try {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/cover-selections/${schoolId}`,
        {
          headers,
          validateStatus: () => true,
        },
      ...
      const clientGrades =
        response.data?.client_grades || response.data?.grades || {};
      ...
      Object.entries(clientGrades).forEach(([gradeKey, entry]) => {
        ...
        if (baselineTheme && !baselineByGrade?.[normalizedGrade]) {
          baselineUpdates[normalizedGrade] = {
            theme: baselineTheme,
  ...
  useEffect(() => {
    ...
    GRADE_ORDER.forEach((gradeKey) => {
      ...
      if (stored?.selectedColourId) {
        const normalizedStoredColour = normalizeColourId(
          stored.selectedColourId,
        );
  ...
  useEffect(() => {
    const fetchThemeImages = async () => {
      ...
      setThemeError("");
      try {
        const response = await axios.get(`${API_BASE_URL}/cover-library`, {
          validateStatus: () => true,
        });
        if (response.status >= 400) {
        ...
        setThemeError(
          "Unable to load theme thumbnails. Uploaded PNGs will appear when the server is reachable.",
        );
        setThemes(buildFallbackThemes());
      } finally {
  ...
  useEffect(() => {
    ...
    const colourForActiveGrade =
      selectedColoursByGrade[grade] || selectedColourId || "";
    const activeColourLabel = colourOptions.find(
      (colour) => colour.id === colourForActiveGrade,
    )?.label;
    ...
    Object.entries(selectedColoursByGrade).forEach(([gradeKey, colourId]) => {
      ...
      const colourLabel = colourOptions.find(
  ...
  const persistStatus = useCallback(
    async (nextStatus, { force = false, suppressToast = false } = {}) => {
      ...
      try {
        ...
        if (didPersistAssignments) {
          ...
          const assignedEntries = Object.values(pngAssignments || {})
            ...
            .map((entry) => {
              ...
              const pngId =
                normalizeColourId(
                  pngName.replace(/\.(png|webp|jpg|jpeg)$/i, ""),
                ) || entry.colourId;
          ...
          const tasks = assignedEntries.map(({ gradeKey, colourId }) =>
            axios.post(
              `${API_BASE_URL}/cover-selections`,
              {
                ...
                colour_id: colourId,
                colour_label:
                  colourOptions.find((c) => c.id === colourId)?.label || null,
                is_selected: true,
              },
              { headers },
            ),
        ...
        await axios[statusMethod](
          `${API_BASE_URL}/cover-status/${schoolId}`,
          { status: nextStatus },
          { headers },
        );
  ...
  const handleThemeSelect = (themeId) => {
    ...
    if (colourSectionRef.current) {
      ...
      setTimeout(() => {
        colourSectionRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }, 0);
  ...
  const showBookSelectionCta =
    isAdmin &&
    typeof onNavigateToBooks === "function" &&
    workflowStatus !== "1";
    ...
    async (gradeKey) => {
      ...
      try {
        ...
        const headers = token
          ...
          : undefined;
        await axios.delete(
          `${API_BASE_URL}/cover-selections/${schoolId}/${normalizedGrade}`,
          { headers },
        );
        ...
        toast.success(
          `Removed ${resolveGradeLabel(normalizedGrade, resolvedGradeNames[normalizedGrade])} selection.`,
        );
      } catch (error) {
  ...
  const renderApprovalGallery = (readonly = false) => {
    ...
    const grouped = approvalCovers.reduce((acc, item) => {
      const gradeKey =
        (item.gradeLabel || "Unknown").toString().trim() || "Unknown";
        ...
        {Object.entries(grouped).map(([gradeLabel, items]) => (
          <div key={gradeLabel} className="space-y-2">
            <div className="text-sm font-semibold text-slate-800 flex items-center gap-2">
              <span className="px-2 py-0.5 rounded bg-slate-100 border border-slate-200 text-xs uppercase tracking-wide">
                {gradeLabel}
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
              {items.map((item) => {
                ...
                const coreTitle =
                  ...
                  item.core_cover_title.toString().trim();
                const workTitle =
                  item.work_cover_title &&
                  item.work_cover_title.toString().trim();
                const addonTitle =
                  item.addon_cover_title &&
                  item.addon_cover_title.toString().trim();
                const displayTitle = (() => {
                  if (component === "work" && workTitle) return workTitle;
                  if (
                    (component === "core" || subject === "assessment") &&
                    coreTitle
                  )
                    return coreTitle;
  ...
  const fetchApprovalAssets = useCallback(async () => {
    ...
    try {
      ...
      const uploadsRes = await axios.get(
        `${API_BASE_URL}/cover-uploads/${school.school_id}`,
        {
          headers,
          validateStatus: () => true,
        },
      ...
      const files = Array.isArray(uploadsRes.data?.files)
        ? uploadsRes.data.files
        : [];
      const missing = Array.isArray(uploadsRes.data?.missing)
        ? uploadsRes.data.missing
        : [];
      const gallery = [
        ...files.map((f) => ({
          name: f.name,
        ...
        ...missing.map((entry) => {
          ...
          const classLabel =
            typeof entry === "string" ? "" : entry?.class_label;
          return {
            name: `${code}.png`,
            url: normalizeAssetUrl(
              `/media/Assets/covers/${school.school_id}/${encodeURIComponent(code)}.png`,
            ),
            exists: false,
            uploadedUrl: null,
      ...
      if (!gallery.length) {
        setApprovalError(
          "No uploaded covers found for the current book selections.",
        );
      }
    } catch (error) {
  ...
  const handleClientApprove = useCallback(async () => {
    ...
    try {
      ...
      const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
      const statusMethod = hasStatusDoc ? "patch" : "post";
      const response = await axios[statusMethod](
        `${API_BASE_URL}/cover-status/${schoolId}`,
        { status: "4" },
        { headers },
      );
  ...
  const clientGradeEntries = useMemo(() => {
    ...
    Object.entries(fetchedGrades.client || {}).forEach(([gradeKey, entry]) => {
      ...
      const baselineColour =
        ...
        entry?.colour_id ||
        "";
      const themeLabel = baselineTheme
        ? getThemeLabel(baselineTheme, entry?.theme_label)
        : "";
            ...
            <Card className="border-none bg-white/80 shadow-lg">
              <CardContent className="p-6 space-y-3">
                <p className="text-lg font-semibold text-slate-900">
                  Cover pages are being prepared
                </p>
                <p className="text-sm text-slate-600">
                  Cover pages are being prepared. Please wait while the team
                  processes them.
                </p>
              </CardContent>
            </Card>
          ...
          <Card className="border-none bg-white/80 shadow-md shadow-orange-100/50">
            <CardHeader className="pb-4">
              <CardTitle className="text-xl font-semibold text-slate-900">
                {workflowStatus === "4"
                  ? "View approved covers"
                  : "Uploaded cover pages"}
              </CardTitle>
              <p className="text-sm text-slate-600">
                {workflowStatus === "4"
            ...
            {isAdmin && (
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleAdminStatusChange("3")}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-base font-semibold text-slate-800">
                  {selectedTheme
                    ? selectedTheme.label
                    : "Select one of the 16 themes to continue"}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
                  ...
                  >
                    ...
                    {isAdmin && (
                      <button
                        type="button"
                        onClick={() =>
                          handleRemoveGradeSelection(entry.gradeKey)
                        }
              ...
              <div className="space-y-6" ref={colourSectionRef}>
                {colourOptions.map((colour, index) => {
                  ...
                  const fullyAssigned =
                    ...
                    thumbnails.every(
                      (thumb) =>
                        pngAssignments[`${colour.id}:${thumb.src}`]?.gradeKey,
                    );
                  return (
                    <div
                      key={colour.id}
                      className={cn(
                        ...
                        fullyAssigned
                          ? "border-orange-500 ring-2 ring-orange-400 ring-offset-2"
                          : "border-slate-200",
                        effectiveReadOnly
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                        {thumbnails.length === 0 ? (
                          ...
                          thumbnails.map((thumb) => {
                            ...
                            const assigned =
                              !!pngAssignments[selectKey]?.gradeKey;
                            return (
                              <div
                                ...
                                <div className="flex items-center justify-between px-2 py-2 gap-2 relative">
                                  <select
                                    ...
                                    className={cn(
                                      ...
                                      !assigned && openSelectKey !== selectKey
                ...
                <div className="space-y-1">
                  ...
                  {effectiveReadOnly && (
                    <p className="text-xs text-slate-500">
                      Viewing mode is enabled because selections are frozen.
                    </p>
                  )}
                </div>
