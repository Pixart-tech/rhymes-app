...
SOCIAL_DIR=(PUBLIC_DIR/"social").resolve()
...
class BookSelectionPayload(BaseModel):
    school_id: str
    personalisation_by_grade: Dict[str, Any] = Field(default_factory=dict)
...
def make_qr_with_logo(link: str, logo_path: str, box_size=12, border=4):
    ...
    logo_size = int(qr_w * 0.22)  # safe size
    ...
    logo_bg.paste(logo, pos, mask=logo)

    # Paste in center
    center = ((qr_w - bg_size) // 2, (qr_h - bg_size) // 2)
    qr_img.paste(logo_bg, center, mask=logo_bg)
...
async def get_binder_json(school_id: str, authorization: Optional[str] = Header(None)):
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        facebook_link=raw_school_record.get("facebook_link")
        instagram_link=raw_school_record.get("instagram_link")
        if facebook_link:
            facebook_qr_image=make_qr_with_logo(facebook_link,SOCIAL_DIR/"facebook.png")
            buf1 = io.BytesIO()
            facebook_qr_image.save(buf1, format="PNG")
            png1 = buf1.getvalue()
            zf.writestr(f"{school_id}_facebook_qr.png", png1)
        if instagram_link:
            
            instagram_qr_image=make_qr_with_logo(instagram_link,SOCIAL_DIR/"instagram.png")
            buf2 = io.BytesIO()
...
async def save_book_selections(
    ...
    for class_key, items in class_groups.items():
        ...
        personalisation_value = (payload.personalisation_by_grade or {}).get(doc_id)
        if isinstance(personalisation_value, bool):
            class_record["personalisation"] = "yes" if personalisation_value else "no"
        elif isinstance(personalisation_value, str):
            normalized_personalisation = personalisation_value.strip().lower()
            if normalized_personalisation in {"yes", "no"}:
                class_record["personalisation"] = normalized_personalisation
